...
Fixpoint min_distance_to_boundaries (x : float64) (boundaries : list VerifiedBoundary) : R :=
  match boundaries with
  | [] => 1000%R
  | b :: rest =>
      let dist := boundary_distance_flocq x b in
      let rest_dist := min_distance_to_boundaries x rest in
      Rmin dist rest_dist
  end.
...
(* NOTE: confidence_monotonic theorem removed to avoid primitive equality issues *)
(* Since confidence_level_flocq always returns 2, monotonicity is trivially true *)